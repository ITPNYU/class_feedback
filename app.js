...
var server_address = "https://itp.nyu.edu/registration/Internal/feedback.php";
//var server_address_menu = "http://itp.nyu.edu/registration/feedback/feedback_test.php";
var classSemester = $("#html_semester").text();
var classYear = $("#html_year").text();
...
function superInit() {
	...
	byId('saveFake').onclick = function() {
		...
		vex.dialog.alert("All feedback saved.");
	}

	byId('saveFake2').onclick = function() {
		// if(readyToLaunch){
		// 	vex.dialog.alert("You've already logged in!");
		// } else {
		// 	doLoginDialog();
		...
		$.getJSON( list_menu(), function(data){
			...
			// if the menu is not updated, use the default
				...
				for(var i=0; i<data.type_of_feedback.length; i++){
					...
					if(!(["TextMiddle","textEnd","textStart"].includes(data.type_of_feedback[i]))){
	...
	$.getJSON( list_student(), function(data){
		...
		$.each(data, function(key, val){
			...
			var $text = $("<textarea>",{
				...
				placeholder: "Put down your feedback to " + val.firstname + ". (E.g., strengths, weakness, opportunities)",
				// width: "80%",
				// height: "5em",
				class: "form-control",
				rows: "6"
